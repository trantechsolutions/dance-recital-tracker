  ...
  if (!showData || !showData.acts) {
    return (
      <div className="text-center py-12 sm:py-20 bg-white dark:bg-slate-800 rounded-xl sm:rounded-2xl border border-slate-100 dark:border-slate-700 shadow-sm space-y-3">
        ...
        <p className="text-slate-400 text-sm px-4">Select a show to search acts and dancers.</p>
      </div>
    );
  }
      ...
      <div className="flex gap-2">
        <div className="relative flex-1">
          ...
          <input
            ...
            className="w-full bg-white dark:bg-slate-800 p-3.5 sm:p-4 pl-10 sm:pl-12 rounded-xl sm:rounded-2xl shadow-sm border border-slate-200 dark:border-slate-700 font-bold text-[15px] sm:text-base dark:text-white outline-none focus:border-pink-500 focus:ring-2 focus:ring-pink-500/10 transition-all"
            value={inputValue}
        ...
        <button
          onClick={handleShare}
          className="bg-white dark:bg-slate-800 w-12 sm:w-[52px] rounded-xl sm:rounded-2xl shadow-sm border border-slate-200 dark:border-slate-700 flex items-center justify-center text-pink-600 hover:bg-pink-50 active:scale-95 transition-all shrink-0"
    ...
    if (actResults.length > 0) {
      return (
        <div className="space-y-2">
          <p className="text-[11px] font-bold text-slate-400 px-0.5">{actResults.length} result{actResults.length !== 1 ? 's' : ''}</p>
...
function DancersPanel({ searchQuery, dancerResults, favoriteDancers, favorites, toggleFavorite, user }) {
  if (searchQuery) {
    if (dancerResults.length > 0) {
      return (
        <div className="space-y-2">
          <p className="text-[11px] font-bold text-slate-400 px-0.5">{dancerResults.length} dancer{dancerResults.length !== 1 ? 's' : ''}</p>
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-2.5 sm:gap-4">
