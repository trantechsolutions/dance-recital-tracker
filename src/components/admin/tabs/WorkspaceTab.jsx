    ...
    <div className="relative">
      ...
      <div className="flex items-center gap-2">
        <button
          ...
          className={clsx(
            "flex items-center gap-2.5 px-4 py-2.5 rounded-2xl border-2 font-bold text-sm transition-all",
            open
              ? "border-pink-500 bg-pink-50 dark:bg-pink-900/20 text-pink-600"
              : orgId
                ? "border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-800 text-slate-700 dark:text-white hover:border-pink-300"
                : "border-dashed border-slate-300 dark:border-slate-600 bg-slate-50 dark:bg-slate-800/50 text-slate-400 hover:border-pink-400"
          )}
        >
          <Building2 size={15} className={orgId ? "text-pink-500" : "text-slate-400"} />
          <span className="max-w-[180px] truncate">{displayName}</span>
          <ChevronDown size={14} className={clsx("transition-transform", open && "rotate-180")} />
        </button>
        {loadingOrgs && <RefreshCw size={14} className="text-slate-400 animate-spin" />}
      </div>

      {/* Dropdown Panel */}
      {open && (
        <div className="absolute top-full left-0 mt-2 w-80 bg-white dark:bg-slate-800 rounded-2xl border border-slate-200 dark:border-slate-700 shadow-xl z-30 overflow-hidden animate-in fade-in zoom-in-95 duration-150">
          <div className="p-3 border-b border-slate-100 dark:border-slate-700 flex items-center justify-between">
            <span className="text-[10px] font-black uppercase tracking-widest text-slate-400">Switch Studio</span>
            <button
          ...
          {isCreating ? (
            <div className="p-4 space-y-3">
              <input
                ...
                className="w-full p-3 rounded-xl bg-slate-50 dark:bg-slate-900 text-slate-900 dark:text-white text-sm font-bold outline-none focus:ring-2 focus:ring-pink-500"
                value={form.name}
                onChange={e => {
              ...
              <div className="relative">
                <input
                  ...
                  className="w-full p-3 pr-10 rounded-xl bg-slate-50 dark:bg-slate-900 text-slate-900 dark:text-white text-xs font-mono outline-none focus:ring-2 focus:ring-pink-500"
                  value={form.id}
                  onChange={e => setForm({ ...form, id: e.target.value.toLowerCase().replace(/[^a-z0-9-]/g, '-') })}
                />
                <span className="absolute right-2.5 top-1/2 -translate-y-1/2 text-[8px] font-black uppercase tracking-wider text-slate-400">ID</span>
              </div>
              <button
                onClick={handleCreate}
                className="w-full bg-pink-600 text-white p-3 rounded-xl font-black text-sm shadow-md active:scale-95 transition-all"
              >
                Create Studio
              </button>
            </div>
            ...
            <div className="max-h-64 overflow-y-auto p-2 space-y-1">
              {loadingOrgs ? (
                ...
                <div className="py-6 text-center text-slate-400 text-xs">No studios found.</div>
              ) : (
                allOrgs.map(org => {
                  ...
                  return (
                    ...
                    >
                      ...
                      {isSelected
                        ...
                        : <ChevronRight size={14} className="text-slate-300 group-hover:text-pink-400 shrink-0 transition-colors" />
                      }
                    </button>
                  );
                })
...
function ManageStudioAccordion({ orgData, setOrgData, showToast, setPromptModal, setSelectedShow, fetchAllOrgs }) {
  ...
  return (
    <div className="bg-white dark:bg-slate-800 rounded-2xl border border-slate-200 dark:border-slate-700 overflow-hidden">
      ...
      {expanded && (
        <div className="border-t border-slate-100 dark:border-slate-700 p-5 space-y-5 animate-in fade-in duration-200">
          {/* Admins Section */}
          <div>
            ...
            <div className="space-y-2 mb-3">
              ...
              {(orgData.admins || []).length === 0 && (
                <p className="text-xs text-slate-400 py-1">No admins configured.</p>
              )}
            </div>
          ...
          {/* Danger Zone */}
          <div className="p-4 rounded-2xl border-2 border-red-200 dark:border-red-900/50 bg-red-50/50 dark:bg-red-900/10">
            <div className="flex items-start gap-3">
              <div className="p-2 bg-red-100 dark:bg-red-900/30 text-red-500 rounded-xl shrink-0">
                <Trash2 size={16} />
              </div>
              <div className="flex-1">
                <h3 className="font-black text-red-600 dark:text-red-400 text-sm mb-0.5">Danger Zone</h3>
                <p className="text-xs text-slate-400 mb-3">
                  Permanently delete <strong className="text-slate-600 dark:text-slate-300">{orgData.name || orgId}</strong> and all its shows, acts, and data. Cannot be undone.
                </p>
                {deleteLog.length > 0 && (
                  <div className="mb-3 bg-red-50 dark:bg-red-900/10 rounded-xl p-3 max-h-28 overflow-y-auto space-y-1">
                    {deleteLog.map((msg, i) => (
...
export default function WorkspaceTab({
  ...
  return (
    <div className="space-y-5 animate-in fade-in">
      ...
      {!orgId && (
        <div className="text-center py-16 bg-white dark:bg-slate-800 rounded-2xl border-2 border-dashed border-slate-200 dark:border-slate-700 animate-in fade-in">
          <Building2 size={48} className="mx-auto text-slate-200 dark:text-slate-700 mb-4" />
          <h3 className="text-lg font-black dark:text-white mb-1">No Studio Selected</h3>
          <p className="text-slate-400 text-sm mb-4">Use the studio picker above to select or create a studio.</p>
        </div>
      )}
      ...
      {orgId && (
        <div className="space-y-4">
          ...
          <div className="flex justify-between items-center">
            <div>
              <h2 className="text-xl font-black dark:text-white">Shows & Acts</h2>
            </div>
            <div className="flex gap-2">
              <button
                onClick={() => { setShowBulkImport(!showBulkImport); setIsAddingShow(false); }}
                className={clsx(
                  "flex items-center gap-2 px-4 py-2.5 rounded-2xl font-bold text-sm transition-all active:scale-95",
                  showBulkImport
              ...
              <button
                ...
                className={clsx(
                  "flex items-center gap-2 px-5 py-2.5 rounded-2xl font-bold text-sm transition-all active:scale-95",
                  isAddingShow
          ...
          {showBulkImport && (
            <div className="bg-white dark:bg-slate-800 p-5 sm:p-6 rounded-2xl border-2 border-amber-400 dark:border-amber-600 shadow-xl animate-in fade-in duration-200">
              <div className="flex items-start gap-3 mb-4">
                <div className="p-2.5 bg-amber-100 dark:bg-amber-900/30 text-amber-600 rounded-xl shrink-0"><Upload size={20} /></div>
                <div className="flex-1">
                  <h3 className="font-black dark:text-white">Bulk Import Shows & Acts</h3>
                  <p className="text-xs text-slate-400 mt-0.5">Upload a single CSV to create multiple shows at once.</p>
                </div>
              ...
              <div className="bg-slate-50 dark:bg-slate-900 p-4 rounded-xl mb-4">
                ...
                <div className="flex items-center justify-between mt-2">
                  ...
                  <button onClick={downloadCsvTemplate} className="flex items-center gap-1 text-[10px] font-bold text-pink-600 hover:text-pink-700 shrink-0 ml-3">
          ...
          {!isAddingShow && showList.length === 0 && (
            <div className="text-center py-14 bg-white dark:bg-slate-800 rounded-2xl border-2 border-dashed border-slate-200 dark:border-slate-700">
              <Calendar size={44} className="mx-auto text-slate-200 dark:text-slate-700 mb-4" />
