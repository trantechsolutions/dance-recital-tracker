  ...
  return (
    <div className="space-y-6 animate-in fade-in">
      ...
      <div className="flex justify-between items-center">
        <div>
          <h2 className="text-2xl font-semibold dark:text-white">Shows & Acts</h2>
        ...
        <div className="flex gap-2">
          <button
            ...
            className={clsx(
              "flex items-center gap-2 px-4 py-3 rounded-card font-bold text-sm transition-all active:scale-95",
              showBulkImport
          ...
          <button
            ...
            className={clsx(
              "flex items-center gap-2 px-5 py-3 rounded-card font-bold text-sm transition-all active:scale-95",
              isAddingShow
      ...
      {showBulkImport && (
        <div className="bg-white dark:bg-ink-800 p-5 sm:p-6 rounded-card border-2 border-amber-400 dark:border-amber-600 shadow-xl animate-in fade-in duration-200">
          <div className="flex items-start gap-3 mb-4">
            <div className="p-2.5 bg-amber-100 dark:bg-amber-900/30 text-amber-600 rounded-card shrink-0">
              <Upload size={20} />
            </div>
            <div className="flex-1">
              <h3 className="font-semibold dark:text-white">Bulk Import Shows & Acts</h3>
              <p className="text-xs text-ink-400 mt-0.5">Upload a single CSV to create multiple shows with all their acts at once.</p>
            </div>
            ...
            <div className="overflow-x-auto">
              <table className="text-xs w-full">
                ...
                <tbody className="text-ink-600 dark:text-ink-300 font-mono text-[11px]">
                  ...
                  <tr className="text-ink-400 dark:text-ink-500"><td className="pr-4 py-0.5">Saturday 2pm</td><td className="pr-4 py-0.5">2</td><td className="pr-4 py-0.5">Jazz Hands</td><td className="py-0.5">Lily T; Zoe G</td></tr>
          ...
          {showList.map(show => {
            ...
            return (
              <button key={show.id} onClick={() => {
                ...
                className={clsx(
                  "text-left p-5 rounded-card border-2 transition-all group",
                  isSelected
                    ? "border-brand-500 bg-brand-50 dark:bg-brand-900/20 shadow-md"
                    : "border-ink-100 dark:border-ink-700 bg-white dark:bg-ink-800 hover:border-brand-300 dark:hover:border-brand-700 hover:shadow-sm"
                )}>
                <div className="flex justify-between items-start">
                  <div className="min-w-0 flex-1">
                    <div className={clsx("font-semibold truncate", isSelected ? "text-brand-600" : "dark:text-white")}>{show.label}</div>
                    {formatShowDate(show.id) && (
                      <div className="text-[10px] font-bold text-ink-400 mt-0.5 flex items-center gap-1"><Calendar size={10} /> {formatShowDate(show.id)}</div>
                    )}
                    <div className="flex items-center gap-3 mt-1.5 text-xs text-ink-400 font-bold">
                  ...
                  <div className={clsx("p-2 rounded-card transition-colors shrink-0", isSelected ? "bg-brand-600 text-white" : "bg-ink-50 dark:bg-ink-900 text-ink-300 group-hover:text-brand-500")}>
                    <Pencil size={16} />
                  </div>
                </div>
              </button>
      ...
      {/* Empty State */}
      {!isAddingShow && showList.length === 0 && (
        <div className="text-center py-16 bg-white dark:bg-ink-800 rounded-card border-2 border-dashed border-ink-200 dark:border-ink-700">
          <Calendar size={48} className="mx-auto text-ink-200 dark:text-ink-700 mb-4" />
...
function SortableActCard({ act, idx, updateAct, onRemove, onMoveUp, onMoveDown, isFirst, isLast }) {
  ...
  return (
    <div ref={setNodeRef} style={style}
      ...
      {/* Reorder controls — prominent full-width buttons on mobile, compact icon column on desktop */}
      <div className="flex lg:flex-col items-stretch lg:items-center justify-center gap-2 lg:gap-1.5 shrink-0">
        <button onClick={onMoveUp} disabled={isFirst} title="Move act up" aria-label="Move act up"
          className="flex-1 lg:flex-none flex items-center justify-center gap-1.5 px-3 py-2.5 lg:p-2 rounded-lg bg-ink-100 dark:bg-ink-700 text-ink-600 dark:text-ink-200 font-bold text-xs hover:bg-brand-500 hover:text-white active:scale-95 transition-all disabled:opacity-30 disabled:cursor-not-allowed disabled:hover:bg-ink-100 dark:disabled:hover:bg-ink-700 disabled:hover:text-ink-600">
          <ChevronUp size={18} /> <span className="lg:hidden">Up</span>
        </button>
        ...
        <button onClick={onMoveDown} disabled={isLast} title="Move act down" aria-label="Move act down"
          className="flex-1 lg:flex-none flex items-center justify-center gap-1.5 px-3 py-2.5 lg:p-2 rounded-lg bg-ink-100 dark:bg-ink-700 text-ink-600 dark:text-ink-200 font-bold text-xs hover:bg-brand-500 hover:text-white active:scale-95 transition-all disabled:opacity-30 disabled:cursor-not-allowed disabled:hover:bg-ink-100 dark:disabled:hover:bg-ink-700 disabled:hover:text-ink-600">
          <ChevronDown size={18} /> <span className="lg:hidden">Down</span>
        </button>
      </div>
